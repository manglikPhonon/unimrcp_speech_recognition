	...
	// Declaring the time variable to calculate the input timer.	
	time_t start_time,end_time,input_timer_duration;
	...
	static mrcp_engine_channel_t* demo_recog_engine_channel_create(mrcp_engine_t *engine, apr_pool_t *pool)
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_engine_channel_create");
	...
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_channel_recognize");
		/* process RECOGNIZE request */
		...
		start_time = time(0);
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"###### Starting Input Timer ######### ");
		...
		if(!recog_channel->audio_out) {
			...
			char *file_name = apr_psprintf(channel->pool,"mrcp_recording_%s.pcm",
	...
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_channel_stop");
		/* process STOP request */
		demo_recog_channel_t *recog_channel = channel->method_obj;
	...
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_channel_timers_start");
		demo_recog_channel_t *recog_channel = channel->method_obj;
		recog_channel->timers_started = TRUE;
	...
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_channel_request_dispatch");
	...
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_start_of_input");
	...
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_result_load");
		FILE *file;
		...
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"######## file_path is : [%s]",file_path);
		...
		char *file_name = apr_psprintf(channel->pool,"mrcp_recording_%s.pcm",
								recog_channel->recog_request->channel_id.session_id.buf);
		char *file_name_wav = apr_psprintf(channel->pool,"mrcp_recording_%s.wav",
								recog_channel->recog_request->channel_id.session_id.buf);
		char *prompt_file_path = apt_vardir_filepath_get(dir_layout,file_name_wav,channel->pool);
		char *prompt_shell = apt_vardir_filepath_get(dir_layout,"pcm2wav.sh",channel->pool);
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"######## dir_layout is  : [%s]",prompt_shell);
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"######## filename is  : [%s]",file_name);
		...
		if(file) {
			...
			apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,text);
			apt_string_assign_n(&message->body,prompt_file_path,size,message->pool);
			fclose(file);
			sprintf(command, "sh %s %s",prompt_shell,file_name);
			x=system(command);
		...
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"######## file is  : [%s]",file);
	...
	{
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_recognition_complete");
		mrcp_recog_header_t *recog_header;
		...
		mrcp_message_t *message = mrcp_event_create(
							...
							recog_channel->recog_request->pool);
		if(!message) {
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"Returning false in recog complete function");
			return FALSE;
		}
		...
		if(recog_header) {
		apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"setting completion cause");
	...
	static apt_bool_t demo_recog_stream_write(mpf_audio_stream_t *stream, const mpf_frame_t *frame)
	{
		//apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_stream_write");
		...
		if(recog_channel->recog_request) {
			...
			switch(det_event) {
				case MPF_DETECTOR_EVENT_ACTIVITY:
					...
					apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"Detected Voice Activity");
				...
				case MPF_DETECTOR_EVENT_INACTIVITY:
					apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"Detected Voice Inactivity " APT_SIDRES_FMT,
						MRCP_MESSAGE_SIDRES(recog_channel->recog_request));
					apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"Detected Voice In Activity");
				...
				case MPF_DETECTOR_EVENT_NOINPUT:
					...
					MRCP_MESSAGE_SIDRES(recog_channel->recog_request));
					apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"Detected no input Activity");
				...
				break;
			default:
					apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"In Default");
 					end_time = time(0);
    					input_timer_duration = end_time - start_time;
					
					if(input_timer_duration == 15) {
					
					apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"Input timer timeout in [%ld] seconds",input_timer_duration);
...
static apt_bool_t demo_recog_msg_signal(demo_recog_msg_type_e type, mrcp_engine_channel_t *channel, mrcp_message_t *request)
{
	apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_msg_signal");
...
static apt_bool_t demo_recog_msg_process(apt_task_t *task, apt_task_msg_t *msg)
{
	apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"INSIDE FUNCTION demo_recog_msg_process");
